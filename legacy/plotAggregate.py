...
def main():
    ...
    module = './' + modname + '/'
    graph_name = modname + '-' + path.split('/')[-1].split('.')[0]
    ...
    percentData = [calcPercentages(d) for d in data]

    # Construct data for graph
    graph_data = {}
    graph_data['Requirement'] = header[2:]
    for d in percentData:
        graph_data[d[0]] = d[1:]
    ...
    plt.savefig(module+'/graphs/'+graph_name+'.pdf')
